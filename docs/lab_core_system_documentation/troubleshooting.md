...
本番向けに同等の初期化を一発で行いたい場合:
- `yarn lab:down-clean`
- これは `core/backend/.env` を残し、DB / generated / runtime apps / runtime appdata / Lab-Core 管理下 Docker 資産を削除します
- 実行時は先に `yarn lab:down` が走り、root 所有ファイルがあっても削除前に自己修復を試みます
- 登録済みアプリが持つ compose project（例: `oruca`）も停止・削除対象です
